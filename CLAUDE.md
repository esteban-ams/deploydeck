...
Run with flags: `./fastship --config config.yaml --port 8000 --version`
...
- **config/** — YAML config parsing with environment variable overrides (`FASTSHIP_PORT`, `FASTSHIP_HOST`, `FASTSHIP_WEBHOOK_SECRET`, `FASTSHIP_LOG_LEVEL`, `FASTSHIP_CLONE_TOKEN`). Token resolution from files (Docker Secrets pattern). Config precedence: CLI flags > env vars > config.yaml > defaults.
- **webhook/** — Echo HTTP handlers, authentication (3 methods: GitHub HMAC, GitLab token, FastShip secret), and push event payload parsing (GitHub/GitLab). Branch filtering for build mode.
...
`cmd/fastship/main.go` — Sets up Echo router, registers routes, and starts the HTTP server.
...
GitHub Actions (`.github/workflows/build.yml`): tests on all PRs/pushes, builds multi-platform binaries on version tags, publishes Docker image to `ghcr.io/esteban-ams/fastship` on main push and tags.
